...
#define  coef(x,y)      coef[1ul*(x)*(t_N+1)+(y)]
  ...
  real *alpha=(real*)_mm_malloc((4*npoints+2)*sizeof(real),64);
  real *coef=(real*)_mm_malloc(1ul*(t_N+1) *(4*npoints+2)*sizeof(real),64);  /*coef is d in eq.6  */ 
  real *powfunction=(real*)_mm_malloc((4*npoints+2)*sizeof(real),64);
  real *gammafunction=(real*)_mm_malloc((4*npoints+2)*sizeof(real),64);
  real *r=(real*)malloc((4*npoints+2)*sizeof(real));
  real *s=(real*)malloc((4*npoints+2)*sizeof(real));   //store the value of 1-(  Jfunction(alpha(i),2)-Jfunction(alpha(i),1) )  
...
begin=MPI_Wtime();
  ...
  for (k=5; k<=nBlock_t*BlockWidth_t; k+=4){              /*compute other value */
	...
	u(i,k+1)= r(i)*(u(i+1,k)+u(i-1,k)) +(s(i)-2*r(i))*u(i,k) + (outputbuffer[(i)*4]) + (Jfunction(alpha(i),(k+1))- Jfunction(alpha(i),k))*u(i,0) ;      
