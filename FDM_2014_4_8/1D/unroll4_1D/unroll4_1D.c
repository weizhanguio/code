...
main(int nargs, char** args)
{
  ...
  */
  ...
  printf("---------unroll 4---coef(x,t)  weizhang----------\n");
...
//	sum_t += p_u[j]*p_coef[k-j];
  ...
  //    p_coef=&(coef[i*(t_N+1)]);
      for (j=1; j<k; j++){
/*	
        sum[i] +=p_u[j]*p_coef[k-j];
        sum1[i]+=p_u[j]*p_coef[k-j+1];
        sum2[i]+=p_u[j]*p_coef[k-j+2];     
        sum3[i]+=p_u[j]*p_coef[k-j+3];    
  */   
         sum[i] +=u(i,j)*coef(i,k-j);
        sum1[i]+=u(i,j)*coef(i,k-j+1);
        sum2[i]+=u(i,j)*coef(i,k-j+2);     
        sum3[i]+=u(i,j)*coef(i,k-j+3);
      ...
      u(i,k+1)= r(i)*(u(i+1,k)+u(i-1,k)) +(s(i)-2*r(i))*u(i,k) + sum[i] + (Jfunction(alpha(i),(k+1))- Jfunction(alpha(i),k))*u(i,0) ;  
    ...
    for (i=1; i<x_N; i++) { 
      sum2[i]+=u(i,k+1)*coef(i,1)+u(i,k)*coef(i,2);
      u(i,k+3)=r(i)*(u(i+1,k+2)+u(i-1,k+2)) +(s(i)-2*r(i))*u(i,k+2) + sum2[i] + (Jfunction(alpha(i),(k+3))- Jfunction(alpha(i),k+2))*u(i,0);
    }
   
   
    for (i=1; i<x_N; i++) { 
      sum3[i]+=u(i,k+2)*coef(i,1)+u(i,k+1)*coef(i,2)+u(i,k)*coef(i,3);
      u(i,k+4)=r(i)*(u(i+1,k+3)+u(i-1,k+3)) +(s(i)-2*r(i))*u(i,k+3) + sum3[i] + (Jfunction(alpha(i),(k+4))- Jfunction(alpha(i),k+3))*u(i,0);
    }
...
/*  
 ...
 // double time=1.0*(finish-begin)/CLOCKS_PER_SEC;   	
  ...
  printf("unroll4: x_N:  %5d, t_N:  %5d   ,Norm_n:  %e, timeusage is %6.2f seconds\n\n",x_N,t_N,sqrt(numer), time);  
