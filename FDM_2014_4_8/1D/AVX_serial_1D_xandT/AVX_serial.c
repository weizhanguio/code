...
#define  coef(x,y)      coef[1ul*(x)*(t_N+1)+(y)]
  ...
  real *alpha=(real*)_mm_malloc((x_N+1)*sizeof(real),64);
  real *coef=(real*)_mm_malloc(1ul*(t_N+1) *(x_N+1)*sizeof(real),64);  /*coef is d in eq.6  */ 
 ...
 */
 ...
 begin=clock();
  ...
  for(i=1;i<=x_N-1;i++)             /*compute  u(:,1)*/
    {        
      u(i,1)=(1-2*r(i))*u(i,0)+r(i)*u(i+1,0)+r(i)*u(i-1,0);//+pow(dt,alpha(i))*tgamma(2-alpha(i))*q(alpha(i),sinfunction(i), gammafunction2(i),t(1)) ;          
  ...
  for (k=1; k<5; k++)              /*compute other value */
    for (i=1; i<x_N; i++) {    
      sum=0.0;
      for (j=1; j<k; j++){
	sum += u(i,k-j) * coef(i,j);
      }    
      u(i,k+1)= r(i)*(u(i+1,k)+u(i-1,k)) +(s(i)-2*r(i))*u(i,k) + sum + (Jfunction(alpha(i),(k+1))- Jfunction(alpha(i),k))*u(i,0) ;//+ powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+1))  ;   
  ...
  for (k=5; k<=nBlock_t*BlockWidth_t; k+=4){              /*compute other value */
    for (i=1; i<=nBlock_x*BlockWidth_x; i+=4) {
      ...
      for (j=1; j<k; j++){
        ...
        */
        ...
        load_u1=_mm256_load_pd((real*)(coef+i*(t_N+1)+k-j)); 
        load_u2=_mm256_load_pd((real*)(coef+(i+1)*(t_N+1)+k-j));
        load_u3=_mm256_load_pd((real*)(coef+(i+2)*(t_N+1)+k-j)); 
        load_u4=_mm256_load_pd((real*)(coef+ (i+3)*(t_N+1)+k-j)); 
	...
	{   
	  ...
	  u(i+m,k+1)= r(i+m)*(u(i+m+1,k)+u(i+m-1,k)) +(1-(  Jfunction(alpha(i+m),2)-Jfunction(alpha(i+m),1) )-2*r(i+m))*u(i+m,k) + (outputbuffer[(i+m)*4]) + (Jfunction(alpha(i+m),(k+1))- Jfunction(alpha(i+m),k))*u(i+m,0) ;//+ powfunction(i+m)*gammafunction(i+m)*q(alpha(i+m),sinfunction(i+m), gammafunction2(i+m),t(k+1))  ;   
    ...
    for (i=nBlock_x*BlockWidth_x+1; i<x_N; i++) {    
      ...
      sum3_rem[i]=0.0;
      for (j=1; j<k; j++){
	sum_rem[i] +=u(i,j) *coef(i,k-j);
        sum1_rem[i]+=u(i,j)*coef(i,k-j+1);
        sum2_rem[i]+=u(i,j)*coef(i,k-j+2);    
        sum3_rem[i]+=u(i,j)*coef(i,k-j+3);     
      }    

   


      u(i,k+1)= r(i)*(u(i+1,k)+u(i-1,k)) +(s(i)-2*r(i))*u(i,k) + sum_rem[i] + (Jfunction(alpha(i),(k+1))- Jfunction(alpha(i),k))*u(i,0) ;//+ powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+1))  ;   
    ...
    for (i=1; i<=nBlock_x*BlockWidth_x; i++) { 
      ...
      u(i,k+2)=r(i)*(u(i+1,k+1)+u(i-1,k+1)) +(s(i)-2*r(i))*u(i,k+1) + outputbuffer[i*4+1] + (Jfunction(alpha(i),(k+2))- Jfunction(alpha(i),k+1))*u(i,0);// + powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+2))  ; 
    ...
    for (i=nBlock_x*BlockWidth_x+1; i<x_N; i++) {  
      ...
      sum1_rem[i]+=u(i,k)*coef(i,1);
      
      u(i,k+2)=r(i)*(u(i+1,k+1)+u(i-1,k+1)) +(s(i)-2*r(i))*u(i,k+1) + sum1_rem[i] + (Jfunction(alpha(i),(k+2))- Jfunction(alpha(i),k+1))*u(i,0) ;//+ powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+2))  ; 
    ...
    for (i=1; i<=nBlock_x*BlockWidth_x; i++) { 
      ...
      u(i,k+3)=r(i)*(u(i+1,k+2)+u(i-1,k+2)) +(s(i)-2*r(i))*u(i,k+2) +outputbuffer[i*4+2] + (Jfunction(alpha(i),(k+3))- Jfunction(alpha(i),k+2))*u(i,0) ;//+ powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+3))  ;
    ...
    for (i=nBlock_x*BlockWidth_x+1; i<x_N; i++) { 
      sum2_rem[i]+=u(i,k)*coef(i,2)+u(i,k+1)*coef(i,1);
      u(i,k+3)=r(i)*(u(i+1,k+2)+u(i-1,k+2)) +(s(i)-2*r(i))*u(i,k+2) + sum2_rem[i] + (Jfunction(alpha(i),(k+3))- Jfunction(alpha(i),k+2))*u(i,0);// + powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+3))  ;
    }
    ...
    for (i=1; i<=nBlock_x*BlockWidth_x; i++) { 
      ...
      u(i,k+4)=r(i)*(u(i+1,k+3)+u(i-1,k+3)) +(s(i)-2*r(i))*u(i,k+3) +outputbuffer[i*4+3]+ (Jfunction(alpha(i),(k+4))- Jfunction(alpha(i),k+3))*u(i,0);// + powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+4))  ;
    ...
    for (i=nBlock_x*BlockWidth_x+1; i<x_N; i++) { 
      sum3_rem[i]+=u(i,k)*coef(i,3)+u(i,k+1)*coef(i,2)+u(i,k+2)*coef(i,1);
      u(i,k+4)=r(i)*(u(i+1,k+3)+u(i-1,k+3)) +(s(i)-2*r(i))*u(i,k+3) + sum3_rem[i] + (Jfunction(alpha(i),(k+4))- Jfunction(alpha(i),k+3))*u(i,0);// + powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+4))  ;
    }
  ...
  for (k=nBlock_t*BlockWidth_t; k<t_N; k++)              /*compute other value */
	...
	sum_t += u(i,j) * coef(i,k-j);
      ...
      u(i,k+1)= r(i)*(u(i+1,k)+u(i-1,k)) +(s(i)-2*r(i))*u(i,k) + sum_t + (Jfunction(alpha(i),(k+1))- Jfunction(alpha(i),k))*u(i,0) ;//+ powfunction(i)*gammafunction(i)*q(alpha(i),sinfunction(i), gammafunction2(i),t(k+1))  ;   
