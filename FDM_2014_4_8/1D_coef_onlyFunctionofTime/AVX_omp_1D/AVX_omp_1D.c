/*
  purpuse: finite difference scheme for variable-order time fractional diffusion equation
  ...
  use intrinsic function! It is a very prelimary version, need further improvement
  date: 2013.9.18
  author: Zhang Wei
*/
...
#define  coef(y)      coef[y]
  ...
  real *coef=(real*)_mm_malloc(1ul* (t_N+1)*sizeof(real),64);  /*coef is d in eq.6  */ 
...
#pragma omp for 
	...
	for(m=0;m<4;m++)
	  {   
	    ...
	    u(i+m,k+1)= r*(u(i+m+1,k)+u(i+m-1,k)) +(1-(  Jfunction(alpha,2)-Jfunction(alpha,1) )-2*r)*u(i+m,k) + (outputbuffer[(i+m)*4]) + (Jfunction(alpha,(k+1))- Jfunction(alpha,k))*u(i+m,0);
